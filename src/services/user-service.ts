...
export class UserService {
  ...
  async registerUser(data: RegisterUserData): Promise<User> {
    ...
    const password = await bcrypt.hash(data.passwordText, 10);
    const newUser = userFactory.create({ email: data.email, password });
  ...
  async validateUserLogin(email: string, passwordInput: string): Promise<User> {
    ...
    const isValid = user && user.password && await bcrypt.compare(passwordInput, user.password);
  ...
  async resetPassword(email: string, code: string, newPasswordText: string): Promise<void> {
      const user = await this.userRepository.findByEmail(email);
      
      const isTokenValid = user && user.resetToken === code && user.resetExpires! > new Date();
