...
export class UsersService {
  ...
  async validateUserLogin(email: string, pass: string) {
    ...
    const isValid = user && user.password && (await bcrypt.compare(pass, user.password));
  ...
  async resetPassword(email: string, code: string, newPass: string) {
    const user = await this.userRepository.findByEmail(email);
    const isValid = user && user.resetToken === code && user.resetExpires && user.resetExpires > new Date();
  ...
  async getAllUsers() { return this.userRepository.getAll(); }
  ...
  private generateToken(user: User) {
    ...
    const secret = this.configService.get<string>('jwt.secret') || 'default_secret_key';
